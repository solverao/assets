...
- `ExtractorService.ExtractAll(src, dest, workers, sync, minFree, removeSource, errorDir)`, `NormalizerService.NormalizeAll(dir, dryRun)` and `ChecksumService.ChecksumAll(dir, outputFile, workers)` take extra args; keep the `process`/`ingest` calls in sync if you change signatures.
